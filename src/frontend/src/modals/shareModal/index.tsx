...
export default function ShareModal({
  ...
  const handleShareComponent = async (update = false) => {
    ...
    function successShare() {
      ...
      setSuccessData({
        title: `${is_component ? "组件" : "工作流"} 共享成功!`,
      });
    }
    ...
    if (!update)
      saveFlowStore(
        ...
        setErrorData({
          title: (is_component ? "组件" : "工作流") + " 分享失败",
    ...
    <>
      ...
      >
        ...
        <BaseModal.Header
          ...
          } 到 WorldSeek Agent 商店。`}
        >
          <span className="pr-2">分享</span>
          <IconComponent
            name="Share3"
          ...
          {open && (
            <>
              ...
              <div className="mb-2 mt-5 flex items-center space-x-2">
                ...
                >
                  将 {nameComponent} 状态设置为公开
                </label>
              </div>
              <span className="text-xs text-destructive">
                <b>注意:</b> 指定字段中的 API 密钥在分享时会自动删除。
              </span>
            </>
          )}
        </BaseModal.Content>
